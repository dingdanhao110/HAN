...
def GLUAttLayer(inputs, attention_size, time_major=False, return_alphas=False):
    ...
    fc = tf.Variable(tf.random_normal([attention_size, hidden_size], stddev=0.1))
    ...
    with tf.name_scope('t'):
        # Applying fully connected layer with non-linear activation to each of the B*T timestamps;
        #  the shape of `v` is (B,T,D)*(D,A)=(B,T,A), where A=attention_size
        t = tf.sigmoid(tf.tensordot(inputs, w_2, axes=1) + b_2)
    vu = t*v  

    # Output of (Bi-)RNN is reduced with attention vector; the result has (B,D) shape
    output = tf.reduce_sum(tf.tensordot(vu, fc, axes=1) , 1)
